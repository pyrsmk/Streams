...
$suite = new MiniSuite\Suite('Dribbble\Project');
...
$suite->expects("limit: 10")
      ->that(function($suite) {
            $elements = [];
            $stream = new Streams\Dribbble\Project('280804-Graphics', $suite['config']);
...
$suite->expects("limit: false")
      ->that(function($suite) {
            ...
            $config = $suite['config'];
            $config['limit'] = false;
            $stream = new Streams\Dribbble\Project('280804-Graphics', $config);
            $stream->get()->then(function($data) use(&$elements) {
                $elements = $data;
            })->wait();
            verifyConsistency($elements);
