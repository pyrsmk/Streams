...
$suite = new MiniSuite\Suite('Vimeo\Channel');
...
$suite->expects("limit: 10")
      ->that(function($suite) {
            $elements = [];
            $stream = new Streams\Vimeo\Channel('comedy', $suite['config']);
...
$suite->expects("limit: false")
      ->that(function($suite) {
            ...
            $config = $suite['config'];
            $config['limit'] = false;
            $stream = new Streams\Vimeo\Channel('comedy', $config);
            $stream->get()->then(function($data) use(&$elements) {
                $elements = $data;
            })->wait();
            verifyConsistency($elements);
...
$suite->expects("nsfw: true")
      ->that(function($suite) {
            ...
            $config = $suite['config'];
            $config['nsfw'] = true;
            $stream = new Streams\Vimeo\Channel('906715', $config);
            $stream->get()->then(function($data) use(&$elements) {
                $elements = $data;
            })->wait();
            verifyConsistency($elements);
...
$suite->expects("nsfw: false")
      ->that(function($suite) {
            $elements = [];
            $stream = new Streams\Vimeo\Channel('906715', $suite['config']);
            $stream->get()->then(function($data) use(&$elements) {
                $elements = $data;
            })->wait();
            verifyConsistency($elements);
